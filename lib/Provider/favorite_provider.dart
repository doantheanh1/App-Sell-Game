...
class FavoriteProvider extends ChangeNotifier {
  ...
  List<String> get library => _libraryIds;
  ...
  // Load library games from firestore
  Future<void> loadLibraryGames() async {
    try {
      QuerySnapshot snapshot = await _firestore.collection("userLibrary").get();
      _libraryIds = snapshot.docs.map((doc) => doc.id).toList();
    } catch (e) {
      print(e.toString());
    }
    notifyListeners();
  ...
  bool isInLibrary(String productId) {
    return _libraryIds.contains(productId);
  }
